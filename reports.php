...
//-------------------- SET SOME VARIABLES ---------------------*/
		...
		$add = "<a href='student-report.php?stud_id=$getarray[stud_id]&class_id=$class_id&inter=2&new=1'>ADD a new report</a>";
      ...
      <!input type='hidden' name='class_id' value='$class_id'>
		...
		//FORM AND DROP DOWN MENUS FOR SELECTION PARAMETERS TO CREATE A NEW REPORT 
		$create_report_form = "
      ...
      <option value='2' label='2'>2</option>
      <option value='3' label='3'>3</option>
		<option value='4' label='4' selected>4</option>
      </select>
       for Year
		<select name='syear' size='0'>
      <option value='9' label='9'>2009-2010</option>
...
//---------------- lastlogin -----------------------//
	...
	$t->set_var(LASTLOGIN, $_SESSION[lastlogin]);
			...
			if($_GET[class_id] != "") {
				...
				$button_printreports = "<a href='student-report-print.php?class_id=$class_id&interim=$interim&syear=$syear&print=1'><img src='img/print.png' border='0' alt=''>&nbsp;&nbsp;VIEW AND/OR PRINT all of the above</a>";
			...
			if($_GET[stud_id] != "") {
				...
				// Since a student in particular is specified, let's associate button/zone to allow creation of new reports for him
				$button_addreport = $create_report_form;
						...
						if($rs->getTotalNum() > "0") {
							...
							while ($getarray = $rs->fetchArray()) {
							   ...
							   $sryear = "$thisyear-$nextyear";
								...
								//cut it short and take off line breaks to make it look better la
								...
								$reports = "$search_specify_form 											
											...
											<!-- SHOW BUTTON/FORM TO CREATE NEXT REPORT (ONLY IF WE ARE DISPLAYING A PARTICULAR STUDENT AT THE MOMENT) -->						
											$button_addreport
											 											
											<!-- PRINT BUTTON STARTS HERE -->
											$button_printreports<br>								
											<!-- PRINT BUTTON STARTS HERE -->
											<br>
											" . $rs->getPageNb("");
